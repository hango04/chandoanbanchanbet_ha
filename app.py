...
def _lazy_cv2():
    try:
        ...
        st.error("❌ Lỗi OpenCV. Trên Streamlit Cloud cần file `packages.txt` chứa `libgl1` và `libglib2.0-0`.")
        raise
...
def softmax(y):
    m = np.max(y[0]); e = np.exp(y[0]-m); s = e / np.sum(e)
...
def load_tflite(path):
    ...
    in_det = inter.get_input_details()[0]; out_det = inter.get_output_details()[0]
    h,w,c = in_det["shape"][1:4]
    def pred(x):
        x2 = x.astype(in_det["dtype"])
        if in_det["dtype"] == np.uint8: x2 = (x2 * 255).astype(np.uint8)
        inter.set_tensor(in_det["index"], x2); inter.invoke()
        y = inter.get_tensor(out_det["index"]); return softmax(y)
    return pred, (h,w,c)
...
def load_model_auto(path, ext):
    ...
    st.error("❌ Sai định dạng model"); return None, None
...
if st.button("🔍 Dự đoán"):
    if not predict_fn:
        ...
        try:
            ...
            if rgb is None: st.error("❌ Lỗi đọc ảnh")
            else:
                probs = predict_fn(x)
                cls = int(np.argmax(probs)); conf = float(np.max(probs))
                st.success(f"✅ **Kết quả:** {LABEL_MAP[cls]} ({conf:.1%})")
                # ghi chữ bằng PIL (không lỗi font)
                im = Image.fromarray(rgb); draw = ImageDraw.Draw(im)
                draw.text((10,10), f"{LABEL_MAP[cls]} {conf:.0%}", fill=(0,255,0))
                st.image(im, caption="Ảnh + dự đoán", width="stretch")
                st.write("### Xác suất:"); 
                for i,p in enumerate(probs[0]): st.write(f"**{i} – {LABEL_MAP[i]}:** {p:.4f}")
        except Exception as e:
            st.exception(e)

st.caption("Nếu vẫn thấy 'Oh no.', mở **Manage app → Logs**: "
           "• `libGL.so.1`: thiếu packages.txt • `Killed`/`OOM`: model quá nặng → dùng .tflite hoặc giảm kích thước model.")
